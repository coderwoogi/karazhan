...
RequestConfirmForType = function(typeLabel, retriesLeft)
  retriesLeft = retriesLeft or 12
  EtherealForge.state.pendingTypeLabel = typeLabel
  EtherealForge.state.pendingTypeKey = GetTypeKey(typeLabel)
  EtherealForge.state.selectedTypeKey = EtherealForge.state.pendingTypeKey
  EtherealForge.state.pendingSlotId =
    EtherealForge.state.pendingSlotId or EtherealForge.state.selectedSlotId
  EtherealForge.state.pendingSlotLabel =
    EtherealForge.state.pendingSlotLabel or EtherealForge.state.selectedSlotLabel

  if EtherealForge.state.transitioning then
    DebugMessage(
      "request type waiting slotId="
        .. tostring(EtherealForge.state.selectedSlotId)
        .. " pendingType=" .. tostring(typeLabel)
    )
    SetStatusText((typeLabel or "선택한 유형")
      .. " 강화 메뉴를 준비하는 중입니다.")
    return
  end
  ...
  local showItemsIndex = FindOptionIndex(options, function(option)
    return string.find(option.cleanText, ROOT_SHOW_ITEMS_TEXT, 1, true) ~= nil
  end)

  DebugMessage(
    "request type page=" .. tostring(page)
      .. " slotId=" .. tostring(EtherealForge.state.selectedSlotId)
      .. " slotAction=" .. tostring(EtherealForge.state.selectedSlotActionIndex)
      .. " showItems=" .. tostring(showItemsIndex)
      .. " transitioning=" .. tostring(EtherealForge.state.transitioning)
  )
  ...
  if EtherealForge.state.selectedSlotActionIndex then
    SetStatusText((typeLabel or "선택한 유형") .. " 강화 확인을 불러오는 중입니다.")
    SelectAction(EtherealForge.state.selectedSlotActionIndex, "type")
    return
  end

  if showItemsIndex and EtherealForge.state.selectedSlotId then
    SetStatusText("장비 목록을 불러와 강화 유형을 연결하는 중입니다.")
    SelectAction(showItemsIndex, "equipment")
    return
  end

  SetStatusText("서버 강화 메뉴와 연결하는 중입니다.")
end
  ...
  if not actionIndex then
    ...
    local showItemsIndex = FindOptionIndex(options, function(option)
      return string.find(option.cleanText,
        ROOT_SHOW_ITEMS_TEXT, 1, true) ~= nil
    end)
    if not showItemsIndex then
      showItemsIndex = EtherealForge.state.rootShowItemsIndex
    end
    if showItemsIndex then
      ...
      HideRequirementItems()
      SetStatusText("강화 유형을 먼저 선택할 수 있습니다. 장비 목록도 함께 불러옵니다.")
      ShowActionButtons({
        {
          label = "[밀리]",
          payload = function()
            RequestConfirmForType("[밀리]")
          end,
        },
        {
          label = "[캐스터]",
          payload = function()
            RequestConfirmForType("[캐스터]")
          end,
        },
        {
          label = "[힐러]",
          payload = function()
            RequestConfirmForType("[힐러]")
          end,
        },
        {
          label = "[탱커]",
          payload = function()
            RequestConfirmForType("[탱커]")
          end,
        },
      })
      if showItemsIndex then
...
local function OpenForOptions(options)
  ...
  if ParsePage(options) == "root" and not EtherealForge.state.autoRouting then
    local showItemsIndex = FindOptionIndex(options, function(option)
      return string.find(option.cleanText,
        ROOT_SHOW_ITEMS_TEXT, 1, true) ~= nil
    end)
...
EtherealForge:SetScript("OnEvent", function(self, event, ...)
  ...
  if event == "GOSSIP_CLOSED" then
    if self.state.transitioning or self.state.autoRouting then
      return
    end
  ...
  if event == "CHAT_MSG_SYSTEM" then
    local message = ...
    if message and (
      string.find(message, "[강화 성공]", 1, true)
      or string.find(message, "[강화 실패]", 1, true)
      or string.find(message, "파괴", 1, true)
      or string.find(message, "강화", 1, true)
    ) then
      SetResultText(message)
      SetStatusText(
        "강화 결과를 확인했습니다. 계속 진행하거나 NPC와 다시 대화해 주세요."
      )
    end
  end
end)
