...
RequestConfirmForType = function(typeLabel)
...
local function ShowEquipmentPage(options)
  ...
  then
    ...
    SetStatusText((EtherealForge.state.pendingTypeLabel or "선택한 유형")
      .. " 강화 확인을 불러오는 중입니다.")
    return SelectAction(EtherealForge.state.selectedSlotActionIndex, "type")
  end
  ...
  then
    ...
    for _, button in ipairs(slotButtons) do
      if button.slotId == pendingSlotId then
        button.actionIndex =
          EtherealForge.state.equipmentActions[pendingSlotId].actionIndex
        return HandleSlotSelection(button)
      end
    end
  end
...
local function ShowTypePage(options)
  ...
  if pendingTypeIndex then
    ...
    return SelectAction(pendingTypeIndex, "confirm")
  end
...
local function OpenForOptions(options)
  ...
  if ParsePage(options) == "root" and not EtherealForge.state.autoRouting then
    if FindOptionIndex(options, function(option)
      return string.find(option.cleanText,
        ROOT_SHOW_ITEMS_TEXT, 1, true) ~= nil
    end) then
