...
RequestConfirmForType = function(typeLabel)
  ...
  if EtherealForge.state.transitioning then
    ...
    After(0.20, function()
      ...
      if EtherealForge.state.pendingTypeLabel == typeLabel then
        RequestConfirmForType(typeLabel)
      end
    end)
    return
  end
  ...
  if ParsePage(options) == "type" then
  ...
  if not actionIndex then
    ...
    if showItemsIndex then
      ...
      After(0.25, function()
        ...
        local refreshOptions = ReadGossipOptions()
        if ParsePage(refreshOptions) == "equipment" then
          ShowCurrentPage(refreshOptions)
        end
      end)
    end
  end
...
local function ShowEquipmentPage(options)
  ...
  then
    ...
    return RequestConfirmForType(EtherealForge.state.pendingTypeLabel)
  end
