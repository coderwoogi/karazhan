...
RequestConfirmForType = function(typeLabel, explicitTypeKey)
  ...
  if not EtherealForge.state.selectedSlotId or not EtherealForge.state.pendingTypeKey then
  ...
  SendForgeCommand("TYPE", tostring(EtherealForge.state.selectedSlotId),
...
HandleSlotSelection = function(self)
  ...
  if not actionIndex then
    ...
    SendForgeCommand("SLOT", tostring(self.slotId))
    return
  end
  ...
  ShowActionButtons({
    {
      ...
      payload = function()
        if EtherealForge.state.selectedSlotId and EtherealForge.state.selectedTypeKey then
          SetResultText("강화 요청을 전송했습니다. 결과를 확인해주세요.")
          SendForgeCommand("DO", tostring(EtherealForge.state.selectedSlotId),
    ...
    },
    {
      ...
      payload = function()
        if EtherealForge.state.selectedSlotId then
          SendForgeCommand("SLOT", tostring(EtherealForge.state.selectedSlotId))
        end
      end,
    },
  })
...
local function ApplyForgePayload(message)
  ...
  if payloadType == "ERROR" then
    SetStatusText(parts[2] or "강화 메뉴를 불러오지 못했습니다.")
    return
  end
end
