    ...
    static bool HandleBetterReloadItemsCommand(ChatHandler* handler, const char* args)
    {
        ...
        for (auto const& itr : Acore::Tokenize(args, ' ', false))
        {
            ...
            QueryResult result = WorldDatabase.Query(
                "SELECT `entry`, `class`, `subclass`, `SoundOverrideSubclass`, `name`, `displayid`, `Quality`, `Flags`, `FlagsExtra`, "
                ...
                "`GemProperties`, `RequiredDisenchantSkill`, `ArmorDamageModifier`, `duration`, `ItemLimitCategory`, `HolidayId`, `ScriptName`, "
            ...
            // ★★★ stat_value 타입: int (DB) → int32 (코드) ★★★
            itemTemplate->StatsCount = 0;
            for (uint8 i = 0; i < MAX_ITEM_PROTO_STATS; ++i)
            {
                itemTemplate->ItemStat[i].ItemStatType = uint32(fields[27 + i * 2].Get<uint8>());
                itemTemplate->ItemStat[i].ItemStatValue = fields[28 + i * 2].Get<int32>();  // ✅ int32!
            ...
            // ★★★ armor: int UNSIGNED (DB) → uint32 (코드) ★★★
            itemTemplate->Armor = fields[55].Get<uint32>();  // ✅ uint32!
            
            // ★★★ 저항: smallint (DB) → int16 (코드) ★★★
            itemTemplate->HolyRes = uint32(fields[56].Get<int16>());    // ✅ int16!
            itemTemplate->FireRes = uint32(fields[57].Get<int16>());
            itemTemplate->NatureRes = uint32(fields[58].Get<int16>());
            itemTemplate->FrostRes = uint32(fields[59].Get<int16>());
            itemTemplate->ShadowRes = uint32(fields[60].Get<int16>());
            itemTemplate->ArcaneRes = uint32(fields[61].Get<int16>());
            ...
            itemTemplate->RandomProperty = fields[109].Get<uint32>();
            ...
            itemTemplate->ScriptId = sObjectMgr->GetScriptId(fields[131].Get<std::string>());
